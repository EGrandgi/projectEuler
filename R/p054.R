...
card_values <- function(h) {
    as.numeric(lapply(h, function(x) {
        card_value(strsplit(x, "")[[1]][1])
    }))
}

same_suit <- function(h) {
    length(c(1:5)[grepl("H", h)]) == 5 | length(c(1:5)[grepl("S", h)]) == 5 | length(c(1:5)[grepl("D", h)]) == 5 | length(c(1:5)[grepl("C", 
        h)]) == 5
}

# Define hands types Ten, Jack, Queen, King, Ace, in same suit
royal_flush <- function(h) {
    cond <- same_suit(h) & grepl("T", toString(h)) == T & grepl("J", toString(h)) == T & grepl("Q", toString(h)) == T & grepl("K", 
        toString(h)) == T & grepl("A", toString(h)) == T
    res <- ifelse(cond, max(card_values(h)), 0)
    res
}

straight_flush <- function(h) {
    # All cards are consecutive values of same suit
    cond <- F
    if (same_suit(h)) {
        vals <- sort(card_values(h))
        if (length(c(1:5)[vals == vals[1]:(vals[1] + 4)]) == 5) {
            cond <- T
        } else {
            cond <- F
        }
